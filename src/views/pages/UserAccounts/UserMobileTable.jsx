...
const UserMobileTable = ({ users = [] }) => {
  const [selectedUserIds, setSelectedUserIds] = useState([]);
  const [selectedIndex, setSelectedIndex] = useState(null);

  if (users.length === 0) {
    return (
      <p className="text-center text-sm text-gray-500">Хэрэглэгч олдсонгүй</p>
    );
  }
      ...
      >
        <p className="font-light text-sm">
          {selectedUserIds.length === 0
            ? "Сонгосон хэрэглэгч байхгүй"
            : `${selectedUserIds.length} хэрэглэгч сонгосон байна`}
        </p>
      </div>
